  ...
  <x-slot name="header">
    <div class="w-full flex justify-between items-center">
      <h2 class="font-semibold text-xl text-gray-800 leading-tight">
        {{ __('Tiket Masuk Wisata Wates Leuwi Ereng Desa Nagrak Selatan') }}
      </h2>
      <div class="p-6 text-gray-900 ">
        <li type="none"><a href="/dashboard/posts/create"
        ...
        <div class="">
          <table class="table-auto w-full">
            ...
            <tbody>
              ...
              @php
              $published_at = \Carbon\Carbon::parse($item->published_at);
              @endphp
              <tr>
                ...
                <td class="border px-4 py-2">
                  {{ $item->status }} |
                  @if ($item->status == 'Paid' && $published_at->diffInDays(now()) < 1) <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-100 text-green-800">Tiket
                    Active</span>
                    @else
                ...
                <td class="border px-4 py-2">
                  <form action="/dashboard/wisata/{{ $item->id }}"" method="post" class="inline">
                    @method('delete')
                    @csrf
                    <button 
